...
# ==========================================
# 2. GESTIÓN DE BASE DE DATOS (NEON POSTGRESQL)
# ==========================================
def get_connection():
    try:
        ...
        st.error(f"Error al conectar con la base de datos de Neon: {e}")
        return None
...
# ==========================================
# 3. SISTEMA DE LOGIN / SESIÓN (ORIGINAL)
# ==========================================
...
if not st.session_state.logged_in:
    st.title("⚡ Iniciar Sesión - App de Rendimiento")
    st.write("Por favor ingresa tu usuario para acceder a tus datos personalizados.")
    
    with st.form("login_form"):
        usuario_input = st.text_input("Usuario / ID de Usuario", placeholder="ej. carlos_fit")
        submitted = st.form_submit_button("Ingresar")
        
        if submitted:
            if usuario_input.strip() != "":
                st.session_state.logged_in = True
                st.session_state.user_id = usuario_input.strip().lower()
                st.success(f"¡Bienvenido {st.session_state.user_id}!")
                st.rerun()
            else:
...
# ==========================================
st.sidebar.title(f"👤 {USER_ID.capitalize()}")
...
if menu == "1. Visión General":
    ...
    st.info(
        "Utiliza la barra lateral para navegar y consultar o registrar tu progreso."
    )
...
# ==========================================
    ...
    with col_chart:
        ...
        if registros_db:
            ...
            st.warning("No hay suficientes datos registrados para mostrar gráficas.")
...
# ==========================================
    ...
    with col_macro:
        st.subheader("Registro Manual de Comidas")
        descripcion = st.text_input("Descripción (ej. 200g pechuga con arroz)")
    ...
    with col_ia:
        ...
        texto_libre = st.text_area(
            ...
            placeholder="Ejemplo: Un tazón de avena con plátano, miel y crema de maní.",
        )
        ...
        if st.button("Calcular Macros con IA"):
            if client and texto_libre:
                ...
                if kcal_calc > 0:
                    ...
                    st.success(
                        f"¡Registrado! {kcal_calc} kcal | {prot_calc}g Proteína"
                    )
            else:
...
def get_connection():
    try:
        # Soporte directo para Neon (obtiene el URL o string de conexión desde st.secrets)
        db_url = st.secrets["postgres"]["url"]
        
        # Neon requiere SSL activo. Si la URL no lo trae, se forza en la conexión.
        if "sslmode" in db_url:
            conn = psycopg2.connect(db_url)
        else:
            conn = psycopg2.connect(db_url, sslmode="require")
...
if not st.session_state.logged_in:
    ...
    st.stop()  # Detiene la ejecución para no mostrar la app hasta iniciar sesión

USER_ID = st.session_state.user_id

# ==========================================
# 4. BARRA LATERAL (NAVEGACIÓN Y LOGOUT)
# ==========================================
st.sidebar.title(f"👤 {USER_ID.capitalize()}")
if st.sidebar.button("Cerrar Sesión"):
    st.session_state.logged_in = False
...
# ==========================================
    ...
    with col_input:
        ...
        if st.button("Guardar Registro"):
            ...
            st.success("¡Registro guardado exitosamente en Neon BD!")
        ...
        if st.button("Calcular Macros con IA"):
            if client and texto_libre:
                ...
                if kcal_calc > 0:
                    ...
                    st.success(
                        f"¡Registrado en Neon! {kcal_calc} kcal | {prot_calc}g Proteína"
                    )
            else:
                st.error("Requiere la API Key de Gemini y un texto de entrada.")
